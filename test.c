...
void test_readFile_more() {
	...
	got = readFile(fd, readed, strlen(buff)+20);
	want = strlen(buff);
...
void test_readFile_dontExist() {
	test = "Read a file that doesnt exist";
	got = readFile(3, test, 0);
	want = -1;
...
void test_readFile_notOpened() {
	...
	test = "Read a file that exists but isnt opened";
	got = readFile(fd, test, 0);
	want = -1;
...
/*
* @Name: File corrupted
* @Description: Add integrity, open without integrity, modify checkFile should return corrupted
*/
void test_file_corrupted() {
	mkFS(RIGHT_DEV_SIZE);mountFS();createFile("test.txt");
	int fd = openFile("test.txt");
	test = "Check file on corrupted file";
	
	includeIntegrity("test.txt");
	char* buff = "Hola\0";
	writeFile(fd, buff, strlen(buff));

	got = checkFile("test.txt");
	want = -1;	
	assertHelper(test, got, want);
}

void test_file_notInitialized() {
	mkFS(RIGHT_DEV_SIZE);mountFS();createFile("test.txt");
	test = "Check file with integrity not initialized";
...
void test_file_withIntegrity(){
	mkFS(RIGHT_DEV_SIZE);mountFS();createFile("test.txt");
	test = "Check file with integrity";
	includeIntegrity("test.txt");
	got = checkFile("test.txt");
	want = 0;	
	assertHelper(test, got, want);
...
void test_openWithIntegrity(){
	mkFS(RIGHT_DEV_SIZE);mountFS();createFile("test.txt");

	test = "Open Integrity file dont exists";
	got = openFileIntegrity("test_.txt");
	want = -1;
	assertHelper(test, got, want);

	test = "Open Integrity file not initialized";
	got = openFileIntegrity("test.txt");
	want = -3;
	assertHelper(test, got, want);

	test = "Open Integrity file with integrity";
	includeIntegrity("test.txt");
	got = openFileIntegrity("test.txt");
	want = 0;
	assertHelper(test, got, want);

	test = "Open Integrity and close without integrity";
	got = closeFile(0);
	want = -1;
	assertHelper(test, got, want);
}

void test_closeWithIntegrity() {

	mkFS(RIGHT_DEV_SIZE);mountFS();createFile("test.txt");

	test = "Close with integrity";
	includeIntegrity("test.txt");
	int fd = openFileIntegrity("test.txt");
	got = closeFileIntegrity(fd);
	want = 0;
	assertHelper(test, got, want);

	createFile("test_.txt");

	test = "Close file with integrity previusly opened without it";
	fd = openFile("test_.txt");
	got = closeFileIntegrity(fd);
	want = -1;
	assertHelper(test, got, want);
...
int main() {
	...
	test_readFile();
	test_readFile_more();
	test_readFile_dontExist();
	test_readFile_notOpened();

	test_file_corrupted();
	test_file_notInitialized();
	test_file_withIntegrity();

	test_openWithIntegrity();
	test_closeWithIntegrity();
